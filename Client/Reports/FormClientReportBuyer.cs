...
{
	/// <summary>
	/// Description of FormClientReportBuyer.
	/// </summary>
	public partial class FormClientReportBuyer : Form
	{
		private ClassMySQL_Full _mySQL = new ClassMySQL_Full();
		private DataSet _dataSet = new DataSet();
		
		public FormClientReportBuyer()
		{
			//
		...
		void FormClientReportBuyerLoad(object sender, EventArgs e)
		{
			//..
		}
		...
		void Button1Click(object sender, EventArgs e)
		{
			...
			_mySQL.SelectSqlCommand = "SELECT * FROM journal WHERE (journal_date BETWEEN '" + dateTimePicker1.Text + "' AND '" + dateTimePicker2.Text + "' AND journal_firm_buyer = '" + textBox3.Text + "' AND journal_type = 'Расходная Накладная' AND journal_delete = 0)";
		...
		void PrintDocument1PrintPage(object sender, System.Drawing.Printing.PrintPageEventArgs e)
		{
			double total = 0;
			// ЗАГОЛОВОК ОТЧЁТА:
			e.Graphics.DrawString("Оборотная ведомость по покупателю.", new Font("Microsoft Sans Serif", 14, FontStyle.Regular), Brushes.Black, 20, 0);
